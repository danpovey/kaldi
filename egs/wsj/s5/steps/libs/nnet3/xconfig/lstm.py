...
class XconfigLstmpLayer(XconfigLayerBase):
    ...
    def generate_lstm_config(self):
        ...
        repair_nonlin_str = "self-repair-scale={0:.10f}".format(repair_nonlin) if repair_nonlin is not None else ''
        repair_clipgrad_str = "self-repair-scale={0:.2f}".format(repair_clipgrad) if repair_clipgrad is not None else ''
        clipgrad_str = "clipping-threshold={0} norm-based-clipping={1} {2}".format(self.config['clipping-threshold'], self.config['norm-based-clipping'], repair_clipgrad_str)
        affine_str = self.config['ng-affine-options']
        ...
        configs.append("component name={0}.r type=ClipGradientComponent dim={1} {2}".format(name, rec_proj_dim, clipgrad_str))
