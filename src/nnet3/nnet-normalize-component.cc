...
void BatchNormComponent::ComputeDerived() {
  ...
  if (!batch_renorm_ || count_eq_zero) {
    ...
    // update the moving averages by dividing the parallel jobs number
    offset_.CopyFromVec(moving_mean_);
    scale_.CopyFromVec(moving_stddv_);
    scale_.ApplyPow(-1.0);
    offset_.MulElements(scale_);
    offset_.Scale(-1.0);
  }
...
BatchNormComponent::BatchNormComponent(const BatchNormComponent &other):
    ...
    r_max_(other.r_max_), d_max_(other.d_max_), batch_renorm_average_count_(other.batch_renorm_average_count_),
...
std::string BatchNormComponent::Info() const {
  ...
  if (count_ > 0) {
    ...
    if (batch_renorm_) {
      Vector<BaseFloat> moving_mean(moving_mean_), moving_stddv(moving_stddv_);
      stream << ", moving-mean=" << SummarizeVector(moving_mean)
             << ", moving-stddv=" << SummarizeVector(moving_stddv);
    }
  }
  return stream.str();
}
